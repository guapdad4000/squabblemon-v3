...
Small red exclamation marks identify sections with activity; six-pixel red dots identify individual unseen items. The header bell opens an accessible native dialog with direct links. Indicators never animate or interrupt play.
...
Item reads persist per player in local storage on this device and sync across tabs. They are presentation state, not reward authorization. Cards clear when inspected, styles when opened, offers when selected. Visiting the bag, bounties, or challenges acknowledges non-claimable updates in that section. The tray offers an explicit bulk acknowledgement; claimable rewards and mail gifts remain until resolved. New mission reset keys and the server's UTC date reintroduce refreshed notices. New offer IDs automatically gain dots. Existing owned items surface on the first visit after this feature ships; players can clear those together.
