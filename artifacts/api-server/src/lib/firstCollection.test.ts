...
import { ROOKIE_DECK_ID, ROOKIE_FOUNDATION_ID, ROOKIE_MENTOR_CORE_IDS, ROOKIE_FOUNDATION_IDS } from '@workspace/squabblemon-engine/data';
...
test('foundation grants survive concurrent bootstrap normalization and repeat requests', { skip: !process.env.DATABASE_URL }, async t => {
  ...
  assert.equal(state.profile.ownedCardIds.length, ROOKIE_FOUNDATION_IDS.length + 1);
