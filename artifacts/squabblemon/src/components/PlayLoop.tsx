...
export function PlayLoop({ mode = 'practice', onExit, initialDeckId = 'block', initialRivalId = 'combo', hideLobby = false, turnTimerEnabled = true, customPlayerDeck, availableDeckIds, storyNodeId }: { mode?: 'guest' | 'practice' | 'tutorial' | 'story'; onExit: () => void; initialDeckId?: string; initialRivalId?: string; hideLobby?: boolean; turnTimerEnabled?: boolean; customPlayerDeck?: Deck; availableDeckIds?: string[]; storyNodeId?: string }) {
  ...
  const timeline = useRef(new PresentationTimeline()), autoStartRef = useRef(false), playerMovesRef = useRef<MatchMove[]>([]), locked = useRef(false);
  ...
  const presentEvents = useCallback(async (resolved: Match, fromSequence: number, id: number, fast = false) => {
    ...
    for (const effect of resolved.effectLog.filter(e => e.sequence >= fromSequence).sort((a, b) => a.sequence - b.sequence)) {
      ...
      if (!await wait(fast ? 0 : reduced ? 70 : 190, id)) return false;
      ...
      if (!await wait(fast ? 0 : reduced ? 90 : 380, id)) return false;
  ...
  const finishRound = useCallback(async (resolved: Match, id: number) => { setPresentationPhase('round-result'); setPhaseMessage(`ROUND ${resolved.round} COMPLETE`); if (await wait(1200, id)) await advanceRoundBoundary(resolved, id); }, [advanceRoundBoundary, wait]);
  const runRival = useCallback(async (afterPlayer: Match, id: number) => { setPresentationPhase('rival-thinking'); setPhaseMessage('RIVAL THINKING'); if (!await wait(700, id)) return; const choice = chooseCpuPlay(afterPlayer); const resolved = choice ? playCard(afterPlayer, 'cpu', choice.instanceId, choice.lane) : pass(afterPlayer, 'cpu'); setMatch(resolved); if (await presentEvents(resolved, afterPlayer.nextEventSequence, id)) await finishRound(resolved, id); }, [finishRound, presentEvents, wait]);
