...
export const shouldRunTurnTimer = (
...
export function PlayLoop({ mode = 'practice', onExit, onTutorialComplete, onVerifiedComplete, initialDeckId = 'block', initialRivalId = 'combo', hideLobby = false, turnTimerEnabled = true, customPlayerDeck, availableDeckIds, storyNodeId, challengeRunId, equippedVariants, cardProgression = {}, activity, draftWeek, draftPicks }: { activity?: string; draftWeek?: string; draftPicks?: string[]; mode?: 'guest' | 'practice' | 'tutorial' | 'story'; onExit: () => void; onTutorialComplete?: () => void; onVerifiedComplete?: (match: Match, serverMatchId?: string) => void; initialDeckId?: string; initialRivalId?: string; hideLobby?: boolean; turnTimerEnabled?: boolean; customPlayerDeck?: Deck; availableDeckIds?: string[]; storyNodeId?: string; challengeRunId?: string; equippedVariants?: EquippedVariantMap; cardProgression?: CardProgressionMap }) {
  ...
  const showMechanicLesson = useCallback(async (lesson: MechanicLesson) => {
    const seen = seenMechanicsRef.current ?? readSeenMechanicLessons(getLessonStorage());
    seenMechanicsRef.current = seen;
    if (seen.has(lesson.id)) return;
    seen.add(lesson.id);
    writeSeenMechanicLessons(seen, getLessonStorage());
    trackEvent('mechanic_lesson_shown', { lesson: lesson.id });
    setMechanicLesson(lesson);
    await new Promise<void>(resolve => {
      mechanicLessonResolverRef.current = resolve;
    });
  }, []);
  const dismissMechanicLesson = useCallback(() => {
    if (mechanicLesson) trackEvent('mechanic_lesson_dismissed', { lesson: mechanicLesson.id });
    setMechanicLesson(null);
    const resolve = mechanicLessonResolverRef.current;
  ...
  const presentEvents = useCallback(async (resolved: Match, fromSequence: number, id: number, fast = false) => {
    ...
    for (const effect of events) {
      ...
      const seenMechanics = seenMechanicsRef.current ?? readSeenMechanicLessons(getLessonStorage());
      seenMechanicsRef.current = seenMechanics;
      let unseenLesson = findFirstUnseenMechanicLesson(effect, seenMechanics);
      while (unseenLesson) {
        await showMechanicLesson(unseenLesson);
  ...
  const commit = useCallback(async (endTurn = false, automatic = false, drop?: { instanceId: string; lane: Lane; squabble: boolean; investment?: number }) => {
    ...
    try {
      ...
      if (challengeRunId) {
        try {
          setPresentationPhase('lock-in'); setPhaseMessage('SAVING YOUR MOVE');
          await saveChallengeCheckpoint(challengeRunId, { moves: candidateMoves }, { signal: AbortSignal.timeout(15000) });
          if (id !== timeline.current.id) return;
        } catch {
          if (id !== timeline.current.id) return;
      ...
      if (playsCard && mode === 'tutorial') {
        setTutorialPlaysByRound(previous => ({
          ...previous,
          [match.round]: (previous[match.round] ?? 0) + 1,
        }));
      }
      ...
      if (automatic && playsCard) {
        ...
        if (challengeRunId) {
          try {
            ...
            if (id !== timeline.current.id) return;
            // The card was already saved. Only the end-turn save failed.
            setMatch(afterPlay); setVisualFrame(afterPlay);
            setSelectedInstanceId(null); setSelectedLane(null); setSquabble(false); setTimerSeconds(TURN_SECONDS);
            setPhaseMessage('CONNECTION LOST · RETRY END TURN');
            setPresentationPhase('player-ready'); locked.current = false;
            return;
          }
        }
  ...
  }, [cancelTimers, challengeRunId, enterPlayerTurn, match, mode, presentationPhase, presentEvents, runRival, selectedInstanceId, selectedLane, setVisualFrame, squabble, tutorialPlaysByRound, waitForBeat]);
  ...
  const tutorialGuidance: TutorialGuidance | null = mode === 'tutorial' && match
    ? getTutorialGuidance({
      match,
      selectedInstanceId,
      selectedLane,
      squabble,
      playsThisRound: tutorialPlaysByRound[match.round] ?? 0,
    })
    : null;
  useEffect(() => {
    if (mode !== 'tutorial' || screen !== 'battle' || presentationPhase !== 'player-ready' || !tutorialGuidance) return;
    const previous = tutorialStepRef.current;
    if (previous?.id === tutorialGuidance.id && previous.round === tutorialGuidance.round) return;
    if (previous) trackEvent('tutorial_step_completed', { round: previous.round, step: previous.id });
    trackEvent('tutorial_step_shown', { round: tutorialGuidance.round, step: tutorialGuidance.id });
    tutorialStepRef.current = { id: tutorialGuidance.id, round: tutorialGuidance.round };
  }, [mode, presentationPhase, screen, tutorialGuidance?.id, tutorialGuidance?.round]);
