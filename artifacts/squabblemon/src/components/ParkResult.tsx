...
export function ParkResult({ outcome, ranked, rank, description, claimed, rivalClaimed, reducedMotion = false, children }: {
  ...
  return <DialogContent ref={panelRef} className={'park-result park-result--illustrated outcome-' + outcome} aria-describedby="park-result-description" data-testid="park-result-dialog">
    ...
    <div className="park-result-scene" style={{ backgroundImage: `linear-gradient(0deg,#18251f00,#09161455),url("${getAssetUrl('assets/fade-park/park.png')}")` }} aria-hidden="true"><div className="park-result-sun" /><motion.img draggable={false} src={getAssetUrl('assets/pvp/dr-fade-' + (win ? 'win' : 'loss') + '.webp')}
      initial={reduced ? false : { y: 45, scale: .85, opacity: 0 }} animate={{ y: 0, scale: 1, opacity: 1 }} transition={{ type: 'spring', damping: 16, delay: .1 }} />
      {outcome !== 'draw' && <img draggable={false} className="park-result-outcome-mark" src={getAssetUrl(`assets/results/${win ? 'win-w' : 'loss-l'}.gif`)} alt="" />}
    </div>
