...
function CardViewComponent({
  ...
  return (
    <><MotionElement
      ...
      aria-label={`${card.name}. ${card.kind === 'token' ? 'Summoned token' : CARD_RARITY_DEFINITIONS[rarity].label + ' rarity'}.${fuseDescription}${covered ? ' Covered until the next targeted hostile ability.' : ''}${disabledReason ? ` ${disabledReason}` : ''}`}
