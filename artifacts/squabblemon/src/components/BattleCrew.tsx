...
export function BattleCrew({ name, crew, onClose, onInspect }: { name: string; crew: CardInstance[]; onClose: () => void; onInspect: (card: CardInstance) => void }) {
  ...
  return <dialog ref={dialog} className="battle-crew-dialog" aria-label={`${name} crew`} onCancel={onClose} onClick={event => { if (event.target === event.currentTarget) onClose(); }}>
    <header><div><span>District formation</span><h2>{name}</h2></div><button autoFocus type="button" onClick={onClose} aria-label="Close crew view">Close</button></header>
