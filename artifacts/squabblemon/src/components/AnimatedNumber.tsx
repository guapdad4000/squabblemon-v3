...
export function AnimatedNumber({ value = 0, from = 0, delay = 0, prefix = '', className = '' }: {
  value: number; from?: number; delay?: number; prefix?: string; className?: string;
}) {
  const node = useRef<HTMLSpanElement>(null);
  const previous = useRef(from);
  const systemReduced = useReducedMotion();
  useEffect(() => {
    const reduced = systemReduced || document.documentElement.dataset.reduceMotion === 'true';
