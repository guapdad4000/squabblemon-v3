...
import { getCardCostExplanation, getDistrictCardBonus, getDistrictResults, getEffectiveCardPower, getLaneScoreForMatch, getLegalCardCost, getRivalIntent, Match, getStoryLockedLanes, getStoryModifierSummaries, getActiveStoryPhase, type EffectLogEntry, type Lane } from '../gameEngine';
    ...
    selectCard(card: Match['playerHand'][number], playableSomewhere: boolean) {
      ...
      if (!playableSomewhere) {
        trackEvent('battle_unavailable_card_selected', {
          ...
          reason: lockedDistricts === districts.length ? 'all_districts_locked' : 'insufficient_motion',
          decision_time: decisionTime(),
        });
      }
...
export function Battle({
  ...
  const participantChange = (participant: any) => {
    ...
    if (!before && after) return `${cardName(participant.cardInstanceId)} entered district ${(after.lane ?? 0) + 1} at ${after.basePower + after.powerModifier} Power`;
    ...
    if (beforePower !== afterPower) changes.push(`Power ${beforePower} → ${afterPower}`);
    ...
    : !selectedCard && selectedLane === null
      ...
      : selectedLane === null
        ...
        : (selectedCost ?? 0) > m.playerMotion
          ...
          : `3. ${districts[selectedLane].name}: ${selectedCost} Motion (${selectedCostDetail}). District bonus +${getDistrictCardBonus(selectedCard, selectedLane)} Power. Lock In${m.squabbleUsed ? '.' : squabble ? ' with SQUABBLE to force a swing.' : ' or save SQUABBLE for a close late district.'}`;
  ...
  const getActionState = () => {
    ...
    if (selectedCard) {
      ...
      return { label: `Lock In · ${selectedCard.name} → ${districts[selectedLane].name} · ${selectedCost} Motion (${selectedCostDetail})${squabble ? ' · SQUABBLE' : ''}`, disabled: false, onClick: commit, type: 'primary', testId: 'button-lock' };
    }
    if (selectedLane !== null) return { label: `Choose Card for ${districts[selectedLane].name}`, disabled: true, type: 'disabled', testId: 'button-pick-card' };
  ...
  return <div data-testid="battle-arena" data-presentation-phase={phase} data-reduced-motion={reducedMotion ? 'true' : 'false'} className={`battle-arena phase-${phase} ${squabble ? 'is-squabble-armed' : ''} flex flex-col h-full w-full max-w-full mx-auto overflow-hidden relative z-10 bg-[#0d0d0d]`} aria-label={`Battle phase: ${phaseMessage}`}>
    ...
    <div className="battle-guidance relative z-30 shrink-0 w-full">
      <div className="min-w-0">
        ...
        {presentedEffect && <div data-testid="effect-causality" className="battle-causality">{triggeredUpgradeName(presentedEffect) && <span data-testid="effect-upgrade-trigger" className="mr-1 font-mono text-[9px] uppercase text-primary">Upgrade · {triggeredUpgradeName(presentedEffect)} · </span>}<b>{cardName(presentedEffect.source?.cardInstanceId ?? presentedEffect.cardInstanceId, presentedEffect.cardId)}</b>{presentedEffect.targetIds.length > 0 ? ` affected ${presentedEffect.targetIds.map(id => cardName(id)).join(', ')}` : ` affected district ${presentedEffect.lane + 1}`}. Score: Rival {presentedEffect.scores.before[presentedEffect.lane]?.cpu ?? 0} / You {presentedEffect.scores.before[presentedEffect.lane]?.player ?? 0} → Rival {presentedEffect.scores.after[presentedEffect.lane]?.cpu ?? 0} / You {presentedEffect.scores.after[presentedEffect.lane]?.player ?? 0}.</div>}
    ...
    <div className="battlefield-grid flex-1 min-h-0 relative z-20">{districts.map((d: any, i: number) => {
      ...
      return <div key={i} data-testid={`lane-container-${i}`} data-squabble-impact={impactPhase && squabbleImpact && impactLane === i ? 'true' : 'false'} className={`district-lane district-lane-${i} min-w-0 relative group ${impactPhase && impactLane === i ? 'district-impact' : ''} ${impactPhase && squabbleImpact && impactLane === i ? 'district-squabble-impact' : ''} ${phase === 'round-result' || phase === 'match-finish' ? `district-verdict verdict-${winner}` : ''} ${selected ? 'is-selected' : ''} ${selectedCard && interactive ? legal ? 'is-legal' : 'is-illegal' : ''} ${activeEffectLane === i ? 'is-effect-lane' : ''} ${lockedLane ? 'is-locked' : ''}`}>
        {activeEffectLane === i && <div className="effect-connection" aria-hidden="true"><span /></div>}
        {impactPhase && impactLane === i && presentedEffect?.kind !== 'story' && <div className={`lane-impact-art kind-${presentedEffect?.kind ?? 'ability'}`} aria-hidden="true" />}
        <div data-testid={`lane-${i}-cpu-zone`} className="battle-side battle-side-rival"><span className="side-mark side-mark-rival">Rival</span><div className="battle-card-stack"><AnimatePresence>{stagedRivalHere && <motion.div key={`back-${stagedRival.instanceId}`} layoutId={stagedRival.instanceId} data-instance-id={stagedRival.instanceId} data-presentation-copy="staged" initial={{ y: -90, rotate: 12, scale: .7, opacity: 0 }} animate={{ y: 0, rotate: -4, scale: 1, opacity: 1 }} className="card-back battle-board-card"><span>S</span></motion.div>}{cpuCards.map((c, j) => <CardView key={c.instanceId} card={c} covered={isCovered(c.instanceId)} isBoard isEnemy disableLayout testId={`card-board-rival-${i}-${c.cardId}-${j}`} onClick={(e) => { e.stopPropagation(); setInspect(c); }} effectivePower={getEffectiveCardPower(c)} {...effectProps(c)} />)}</AnimatePresence></div></div>
        <button type="button" data-testid={`lane-${i}`} onClick={() => decisionHandlers.selectDistrict(i, available)} aria-pressed={selected} aria-disabled={!available} tabIndex={interactive ? 0 : -1} aria-label={selectedCard ? `${legal ? 'Deploy' : lockedLane ? 'Cannot deploy, district locked' : `Cannot deploy, need ${laneCost} Motion`} ${selectedCard.name} to ${d.name}` : `${selected ? 'Selected' : lockedLane ? 'Cannot select, locked' : 'Select'} ${d.name} district first`} title={lockedLane ? 'This district is locked this round.' : selectedCard && !affordable ? `Need ${laneCost} Motion; you have ${m.playerMotion} (${laneCost! - m.playerMotion} short).` : undefined} className="district-target focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"><div className="district-marker"><div className="district-score-row"><motion.span key={`cpu-${cScore}`} initial={{ scale: 1.45 }} animate={{ scale: 1 }} data-testid={`score-cpu-${i}`} className={cScore > pScore ? 'text-accent' : 'text-white/55'}>{cScore}</motion.span><span className="score-divider">:</span><motion.span key={`player-${pScore}`} initial={{ scale: 1.45 }} animate={{ scale: 1 }} data-testid={`score-player-${i}`} className={pScore > cScore ? 'text-primary' : 'text-white/55'}>{pScore}</motion.span></div><div className="district-kicker">0{i + 1} · {pScore > cScore ? 'You lead' : cScore > pScore ? 'Rival leads' : 'Tied'}</div><h3>{d.name}</h3><p>{d.rule}</p>{lockedLane ? <span className="district-prompt is-blocked">Locked this round</span> : interactive && <span className={`district-prompt ${selected && legal ? 'is-ready' : selected ? 'is-selected' : selectedCard && !affordable ? 'is-blocked' : ''}`}>{selected && legal ? `Ready · ${laneCost} Motion` : selected ? 'District selected' : selectedCard ? affordable ? `Play · ${laneCost} Motion` : `Need ${laneCost} · ${laneCost! - m.playerMotion} short` : 'Tap to select first'}</span>}</div></button>
        <div data-testid={`lane-${i}-player-zone`} className="battle-side battle-side-player"><span className="side-mark side-mark-player">You</span><div className="battle-card-stack"><AnimatePresence>{stagedPlayerHere && <motion.div key={`player-back-${stagedPlayer.instanceId}`} data-instance-id={stagedPlayer.instanceId} data-presentation-copy="staged" initial={{ y: 90, rotate: -10, scale: .72, opacity: 0 }} animate={{ y: 0, rotate: 3, scale: 1, opacity: 1 }} className="card-back battle-board-card"><span>S</span></motion.div>}{playerCards.map((c, j) => <CardView key={c.instanceId} card={c} covered={isCovered(c.instanceId)} variantId={getEquippedVariant(equippedVariants, c.id)} isBoard disableLayout squabble={impactPhase && squabbleImpact && activeEffectId === c.instanceId} testId={`card-board-player-${i}-${c.cardId}-${j}`} onClick={(e) => { e.stopPropagation(); setInspect(c); }} effectivePower={getEffectiveCardPower(c)} {...effectProps(c)} />)}</AnimatePresence></div></div>
      </div>;
    })}</div>
    <div data-testid="battle-command-deck" inert={interactive ? undefined : true} className="battle-command-deck shrink-0 relative z-40"><div id="hand-tray" data-testid="hand-tray" className="battle-hand-tray"><div className="battle-hand-row"><AnimatePresence>{m.playerHand.filter(c => c.instanceId !== stagedPlayer?.instanceId).map(c => { const cardCost = selectedLane !== null ? getLegalCardCost(m, 'player', c, selectedLane as Lane) : c.cost; const unlockedLanes = ([0, 1, 2] as Lane[]).filter(lane => !lockedLanes.includes(lane)); const playableSomewhere = unlockedLanes.some(lane => getLegalCardCost(m, 'player', c, lane) <= m.playerMotion); const playableForChoice = selectedLane === null ? playableSomewhere : !lockedLanes.includes(selectedLane as Lane) && cardCost <= m.playerMotion; const cheapestCost = unlockedLanes.length ? Math.min(...unlockedLanes.map(lane => getLegalCardCost(m, 'player', c, lane))) : c.cost; const neededCost = selectedLane !== null ? cardCost : cheapestCost; const reason = lockedLanes.length === districts.length ? 'Cannot play: every district is locked this round.' : !playableForChoice ? `Cannot play: costs ${neededCost} Motion${selectedLane !== null ? ` in ${districts[selectedLane].name}` : ''}; you have ${m.playerMotion} (${Math.max(0, neededCost - m.playerMotion)} short).` : undefined; return <CardView key={c.instanceId} card={c} variantId={getEquippedVariant(equippedVariants, c.id)} queued={selectedInstanceId === c.instanceId} squabble={squabble && selectedInstanceId === c.instanceId} cost={cardCost} unavailable={interactive && !playableForChoice} disabledReason={reason} onClick={(e) => { e.stopPropagation(); decisionHandlers.selectCard(c, playableForChoice); }} className="origin-bottom" />; })}</AnimatePresence></div></div>
      ...
      <div className="battle-actions"><div className="battle-motion"><div>Your Motion</div><strong data-testid="motion-player">{m.playerMotion}</strong></div><button data-testid="button-squabble" title={m.squabbleUsed ? 'SQUABBLE has already been used.' : !selectedCard ? 'Choose a card first.' : 'Double this card’s base Power once per match.'} className={`battle-squabble ${squabble ? 'is-armed' : ''}`} onClick={() => decisionHandlers.toggleSquabble(selectedCard ?? null)} disabled={m.squabbleUsed || !interactive || !selectedCard}>{m.squabbleUsed ? 'Squabble spent' : squabble ? 'Squabble armed' : 'Arm Squabble'}</button><button data-testid={action.testId} onClick={action.onClick} disabled={action.disabled} className={`battle-primary-action action-${action.type}`}>{action.label}</button></div>
    </div>
  </div>;
}
/*
export function Battle({
  ...
  return <div className={`battle-arena phase-${phase} flex flex-col h-full w-full max-w-full mx-auto overflow-hidden relative z-10 bg-[#0d0d0d]`} aria-live="polite" aria-label={`Battle phase: ${phaseMessage}`}>
    ...
    <div inert={!interactive} className="battle-command-deck shrink-0 relative z-40"><div id="hand-tray" data-testid="hand-tray" className="battle-hand-tray"><div className="battle-hand-row"><AnimatePresence>{m.playerHand.filter(c => c.instanceId !== stagedPlayer?.instanceId).map(c => { const playableSomewhere = ([0, 1, 2] as Lane[]).some(lane => !lockedLanes.includes(lane) && getLegalCardCost(m, 'player', c, lane) <= m.playerMotion); const reason = !playableSomewhere ? `Cannot play now. Need more Motion or an unlocked district.` : undefined; return <CardView key={c.instanceId} card={c} variantId={getEquippedVariant(equippedVariants, c.id)} queued={selectedInstanceId === c.instanceId} squabble={squabble && selectedInstanceId === c.instanceId} cost={selectedLane !== null ? getLegalCardCost(m, 'player', c, selectedLane as Lane) : c.cost} unavailable={interactive && !playableSomewhere} disabledReason={reason} onClick={(e) => { e.stopPropagation(); decisionHandlers.selectCard(c, playableSomewhere); }} className="origin-bottom" />; })}</AnimatePresence></div></div>
      {selectedCard && selectedLane !== null && interactive && <div className={`target-trajectory target-lane-${selectedLane}`} aria-hidden="true"><span /></div>}
      <div className="battle-actions"><div className="battle-motion"><div>Motion</div><strong data-testid="motion-player">{m.playerMotion}</strong></div><button data-testid="button-squabble" title={m.squabbleUsed ? 'SQUABBLE has already been used.' : !selectedCard ? 'Choose a card first.' : 'Double this card’s base Power once per match.'} className={`battle-squabble ${squabble ? 'is-armed' : ''}`} onClick={() => decisionHandlers.toggleSquabble(selectedCard ?? null)} disabled={m.squabbleUsed || !interactive || !selectedCard}>{m.squabbleUsed ? 'Squabble spent' : squabble ? 'Squabble armed' : 'Arm Squabble'}</button><button data-testid={action.testId} onClick={action.onClick} disabled={action.disabled} className={`battle-primary-action action-${action.type}`}>{action.label}</button></div>
    </div>
  </div>;
}
*/
