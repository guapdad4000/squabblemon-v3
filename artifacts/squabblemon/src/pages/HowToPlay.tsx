...
import { CARD_RARITIES, CARD_RARITY_DEFINITIONS, cardCatalog, catalogCardById, ROOKIE_CORE_IDS, ROOKIE_FOUNDATION_IDS } from '../data';
import { CARD_LEVEL_CAP, totalXpForCardLevel } from '@workspace/squabblemon-engine/cardProgression';
import { MOVE_TRAINING_COSTS, SHOP_OFFERS, battleEarnings } from '@workspace/squabblemon-engine/economy';
...
export default function HowToPlay() {
  ...
  return <div className="how-to-play">
    ...
    <header className="guide-header" id="guide-top">
      <Link href="/" aria-label="Squabblemon home"><img src={getAssetUrl('brand/squabblemon-wordmark.webp')} alt="Squabblemon" /></Link>
      <span className="guide-header-label"><BookOpen size={14} /> THE FIELD GUIDE</span>
      <Link className="guide-header-play" href="/play/guest">Try a match <ArrowRight size={16} /></Link>
    </header>
    <main>
      <section className="guide-hero" aria-labelledby="guide-title">
        ...
        <div className="guide-hero-copy"><span className="guide-eyebrow">THE RULES. THE ROSTER. THE COME UP.</span>
          ...
          <p>Build your crew. Pull someone unexpected. Turn your favorites into neighborhood legends. Here’s how to play Squabblemon.</p>
        ...
        <div className="guide-hero-cast" aria-label="Squabblemon characters Rastamon, Hooper, and OG Uncle">
          <span className="guide-hero-orbit" /><span className="guide-hero-tag">YOUR CREW.<br />YOUR REPUTATION.</span>
          ...
          <span className="guide-hero-stamp"><b>7 CARDS</b><span>ONE CREW</span></span>
        </div>
        <div className="guide-hero-facts"><span><b>07</b> cards in your crew</span><span><b>03</b> districts to contest</span><span><b>06</b> rounds to make it count</span><span><b>02</b> districts to win</span></div>
      </section>
      ...
      <section className="guide-section guide-loop" id="the-loop" aria-labelledby="loop-heading">
        ...
        <div className="guide-loop-steps">{[
          ['deck-stack', 'Build your crew', 'Pick ten unique cards. Mix affordable openers, useful support, and a closer.'],
        ...
        <div className="guide-inline-note"><Check size={19} /><p><strong>Start with a real crew.</strong> Rookie Road gives you a {ROOKIE_FOUNDATION_IDS.length}-card foundation. You can learn the game before chasing the rest of the collection.</p></div>
      </section>

      <section className="guide-section guide-paper" id="your-crew" aria-labelledby="crew-heading">
        <div className="guide-section-intro"><SectionTitle number="02" label="Characters & crew building"><span id="crew-heading">Pick personalities.<br /><em>Build a plan.</em></span></SectionTitle><p>{cardCatalog.length} characters. Different costs, different abilities, different ways to win. Choose an example below to see what a character brings to your crew.</p></div>
        <CharacterLab />
        <div className="guide-deck-lesson"><div><span className="guide-eyebrow">A first crew you can grow with</span><h3>Ten cards.<br />More than one way out.</h3><p>This Rookie Road lineup gives you cheap plays, a discount, freeze, protection, a cleanse, disruption, and a comeback card.</p><Link href="/game/decks" className="guide-text-link">Open your crew builder <ArrowRight size={17} /></Link></div>
          <div><div className="guide-rookie-lineup">{ROOKIE_CORE_IDS.map(id => <figure key={id}><img src={getCardImage(id)} alt="" loading="lazy" /><figcaption>{catalogCardById[id].name}<small>{catalogCardById[id].cost} Motion</small></figcaption></figure>)}</div><p className="guide-small-note">Crew rule: exactly ten different owned cards. Characters can mix across crews and types. Keep some 1–2 Motion plays so you have options early.</p></div>
        </div>
      </section>

      <section className="guide-section guide-battle" id="the-battle" aria-labelledby="battle-heading">
        <div className="guide-section-intro"><SectionTitle number="03" label="Playing a match"><span id="battle-heading">Win the districts.<br /><em>Win the argument.</em></span></SectionTitle><p>At the end of round six, you need more Hands in at least two districts. A giant stack in one district still wins only one district.</p></div>
        <div className="guide-board" aria-label="Example final board: you win districts one and three, taking the match two to one">
          ...
          <div className="guide-board-verdict"><strong>2–1. YOUR BLOCK.</strong><p>You win with 26 total Hands against 34. District control decides the match.</p></div>
        </div>
        <div className="guide-battle-rules"><article><span>01 / DRAW & READ</span><h3>Start with five.</h3><p>Your crew has ten cards. A normal match opens with five in hand, then draws one each round until the deck is empty. Read the board before committing.</p></article><article><span>02 / SPEND MOTION</span><h3>Spend your turn wisely.</h3><p>Drag a card into a lane and release to play it. You can also tap to select a card and lane. Hold any card to read its details without playing it. On desktop, right-click or press Alt+Enter. Play while you can afford it, then end your turn. You start with 2 Motion. Later rounds give Motion equal to the round number, plus up to 1 unspent Motion carried over, capped at 9.</p></article><article><span>03 / WATCH THE REVEAL</span><h3>Abilities change things.</h3><p>Your card resolves, then the opponent responds. Follow the updated district scores. Freeze, protection, movement, and other effects can change the value of a play.</p></article></div>
        <div className="guide-squabble"><div className="guide-squabble-symbol">×2</div><div><span className="guide-eyebrow">Once per match</span><h3>Make it a SQUABBLE.</h3><p>Use SQUABBLE when playing a card to double its <strong>base Hands</strong>. Its Motion cost still applies. Save it for a play that helps secure a second district.</p></div><span className="guide-squabble-example">Hooper’s base Hands<b>5 → 10</b><small>Ability effects resolve separately.</small></span></div>
        <div className="guide-status-notes"><p><strong>Frozen?</strong> The card contributes 0 Hands until cleansed.</p><p><strong>Silenced?</strong> Its ability is disabled; its Hands can still count.</p><p><strong>Tied?</strong> A tied district belongs to neither player. If neither owns two at the finish, the match is a draw.</p></div>
        <p className="guide-small-note">Three locations are drawn from sixteen each match. Bonuses, penalties, movement, and protection can change your plan. All rules are visible before you play; previews include their effects. Location penalties cannot push a card below 0 Hands. Story encounters can add modifiers.</p>
      ...
      <section className="guide-section guide-packs" id="street-packs" aria-labelledby="packs-heading">
        <div className="guide-pack-intro"><div><SectionTitle number="04" label="Gacha / Street Packs"><span id="packs-heading">New faces.<br /><em>Fresh possibilities.</em></span></SectionTitle><p>A Street Pack gives you five different card pulls plus a bonus reward. Spend a ticket or use your earned Clout, then reveal what joins your collection.</p><div className="guide-pack-price"><span><Ticket size={20} /><b>1</b> ticket</span><small>OR</small><span><b>{offerPrice('ticket')}</b> Clout</span></div><p className="guide-small-note">Two new cards guaranteed while missing cards remain. Owned pulls become Style Shards.</p></div><div className="guide-pack-art"><span className="guide-pack-halo" /><img src={getAssetUrl('assets/props/foil-pack.webp')} alt="Squabblemon gold and black Street Pack" loading="lazy" /><span className="guide-pack-sticker">5 CARDS + BONUS<br />IN EVERY PACK</span></div></div>
        <div className="guide-pack-slots"><article><span className="guide-slot-number">01–02</span><span className="guide-eyebrow">The recruits</span><h3>Two new faces.</h3><p>Each pull guarantees a new card while a missing card remains. Once you own the full pool, owned pulls become 25 Style Shards.</p><strong>COLLECTION PROTECTION</strong></article><article><span className="guide-slot-number">03–05</span><span className="guide-eyebrow">Build your crew</span><h3>Three more card pulls.</h3><p>Every pack pulls five different cards. Each uses the rarity weights below; a rarity with no eligible cards falls back to the remaining pool. Owned cards convert to 25 Style Shards.</p><strong>NO REPEATS WITHIN A PACK</strong></article><article><span className="guide-slot-number">06</span><span className="guide-eyebrow">The bonus</span><h3>A little extra.</h3><ul><li><b>70%</b> 15, 25, or 40 Style Shards</li><li><b>25%</b> 50 or 100 Clout</li><li><b>5%</b> Cosmetic style</li></ul><strong>BASE ODDS / BEFORE STYLE GUARANTEE</strong></article></div>
        <div className="guide-pack-protection"><div><Layers3 size={24} /><h3>Already own that card?</h3><p>A duplicate converts to <strong>25 Style Shards</strong>. You don’t need duplicate copies to train a character.</p></div><div><Sparkles size={24} /><h3>A style by pack ten.</h3><p>If nine openings pass without a style, the tenth awards an unowned style in the bonus slot. A style pull resets the counter. This guarantee covers cosmetics; Legendary and Mythical characters have no rarity guarantee.</p></div></div>
        <details className="guide-disclosure"><summary>A few details behind the reveal <ChevronRight size={18} /></summary><div><p>The first two pulls each roll against the cards you are missing. If a rolled rarity has no missing characters, it picks uniformly from all remaining missing characters. Remaining pulls exclude cards already pulled in this pack. Effective rarity odds can change with your collection and which cards remain eligible.</p><p>A cosmetic pull does not unlock the character it belongs to. You can equip it after owning that character. Once all styles are owned, the style guarantee ends and the normal 5% style result becomes 50 Style Shards.</p></div></details>
      ...
      <section className="guide-section guide-first-session" id="first-session" aria-labelledby="start-heading">
        ...
        <div className="guide-first-copy"><SectionTitle number="07" label="Your first session"><span id="start-heading">You’re up,<br /><em>rookie.</em></span></SectionTitle>
          <ol><li><b>Meet Dr. Fade.</b><p>Create an account and follow Rookie Road to receive your foundation.</p></li><li><b>Save your ten.</b><p>Build a legal crew. Keep cheap options and a plan for two districts.</p></li><li><b>Finish practice. Claim the welcome reward.</b><p>Your 250 welcome Clout can cover a 100-XP Practice Session and the first 150-Clout move coaching for one new character.</p></li><li><b>Take it back to the streets.</b><p>Play story, claim ready bounties, and try your new move. Save for a pack or choose a Common when you know what your crew needs.</p></li></ol>
          <div className="guide-start-actions"><Link href="/game" className="guide-button guide-button--yellow">Build my crew <ArrowRight size={18} /></Link><Link href="/play/guest" className="guide-text-link">Try guest practice <ArrowRight size={16} /></Link></div>
        </div>
      </section>
      <section className="guide-section guide-faq" aria-labelledby="faq-heading"><div><span className="guide-eyebrow">One last thing</span><h2 id="faq-heading">The quick answers.</h2></div><div>
        {[
          ...
          ['Does a duplicate upgrade my card?', 'No. A duplicate becomes 25 Style Shards. Character upgrades use XP and Clout-funded coaching, so you can keep training the copy you already own.'],
    ...
    <footer className="guide-footer"><img src={getAssetUrl('brand/squabblemon-crest.webp')} alt="" loading="lazy" /><div><strong>KNOW THE BLOCK. MAKE YOUR MOVE.</strong><p>Squabblemon field guide · Current game rules · Balance values may evolve.</p></div><a href="#guide-top">Back to top ↑</a></footer>
  </div>;
}
