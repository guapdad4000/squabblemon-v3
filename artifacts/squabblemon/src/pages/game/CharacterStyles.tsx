...
import { CHARACTER_STYLE_OFFERS, CHARACTER_STYLE_ROLLOUT, ownsStyle, styleSetFor, validateCosmeticLoadout, type CosmeticLoadout, type CharacterStyleOfferId } from '@workspace/squabblemon-engine/cosmetics';
...
export function CharacterStyles({ bootstrap, cardId = 'kyle' }: { bootstrap: PlayerBootstrap; cardId?: string }) {
  ...
  return <main className="character-styles" data-testid="character-styles">
    <header className="character-styles__header"><div><Link href="/game/inventory" className="style-link">← Back to the bag</Link><p className="style-kicker">SIGNATURE COLLECTION / 001</p><h1>{card.name}<i>Make it yours.</i></h1></div><div className="style-wallet"><GameGlyph name="shards"/><strong>{profile.styleShards.toLocaleString()}</strong><span>Style Shards</span></div></header>
    ...
    <div className="style-workbench"><section className="style-workbench__art" aria-label="Cosmetic preview">
      {tab === 'stickers' ? <><div className="style-sticker-sheet">{set.stickers.map(sticker => <button type="button" key={sticker.id} disabled={!unlocked || busy || !!pending || (!stickers.includes(sticker.id) && stickers.length >= 3)} aria-pressed={stickers.includes(sticker.id)} aria-label={sticker.name} onClick={() => toggleSticker(sticker.id)}><CharacterSticker id={sticker.id} decorative/><span>{sticker.name}</span><small>{stickers.includes(sticker.id) ? 'ON YOUR BANNER' : 'DIE-CUT / ' + String(sticker.cell + 1).padStart(2,'0')}</small></button>)}</div><p>Four originals. One complete pack. {unlocked ? stickers.length + ' / 3 banner slots filled.' : 'Every sticker shown is included.'}</p></> : tab === 'banner' ? <div className="style-banner-options"><CharacterBanner cardId={cardId} finish={finish} compact/><div>{(['base','silver'] as const).map(value => <button type="button" key={value} className="style-button" aria-pressed={finish === value} onClick={() => setFinish(value)}>{value === 'base' ? 'Blue Hour · Included' : 'Silver Lining · 120 shards'}</button>)}</div><p>A layered character banner for your profile and inventory bag.</p></div> : <div className="style-card-scene"><div><CardView card={card} backgroundUrl={getAssetUrl(set.background)} fillContainer presentationOnly disableLayout /></div><p>Blue Hour / Rooftop court<br/><small>Preview on your actual card.</small></p></div>}
    </section><aside className="style-receipt"><span className="style-kicker">{tab === 'stickers' ? 'THE WHOLE PACK' : tab === 'banner' && finish === 'base' ? 'PART OF YOUR CHARACTER' : 'THE FINISHING TOUCH'}</span><h2>{tab === 'banner' && finish === 'base' ? 'Blue Hour banner' : offer.name}</h2><p>{tab === 'banner' && finish === 'base' ? 'Your character. Your name. Your favorite stickers. Included when KYLE joins your gang.' : offer.description}</p>
      <ul>{(tab === 'stickers' ? set.stickers.map(sticker => sticker.name) : tab === 'banner' ? ['Original character artwork','Animated layered banner','Respects reduced motion'] : ['Hand-painted rooftop scene','Works with your foil finish','Collection, details and your cards']).map(text => <li key={text}>{text}</li>)}</ul>
      ...
      {!owned ? <p className="style-notice">Unlock {card.name} to collect his cosmetics.</p> : null}
    ...
    <details className="style-rollout"><summary>Up next · Mythicals, Legendaries, Super Rares</summary><p>KYLE starts the series. More signature collections are in development.</p><div>{CHARACTER_STYLE_ROLLOUT.filter(item => !item.available).map(item => <span key={item.cardId}><strong>{item.name}</strong><small>{CARD_RARITY_DEFINITIONS[item.rarity].label} · In development</small></span>)}</div></details>
  </main>;
}
