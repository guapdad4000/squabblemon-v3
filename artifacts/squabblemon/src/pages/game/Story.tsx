...
export function Story({ bootstrap }: { bootstrap: PlayerBootstrap }) {
  ...
  return <div className="studio-page story-atlas">
    <header className="story-atlas__header"><div><nav className="studio-tabs" aria-label="Chapters">{campaign.chapters.map(chapter=><button key={chapter.id} disabled={chapter.status==='locked'} aria-pressed={chapter.id===currentChapter?.id} onClick={()=>setActiveChapterId(chapter.id)}>Chapter {chapter.order || 1}{chapter.status==='locked' && <LockKeyhole size={10}/>}</button>)}</nav><span className="studio-eyebrow">{currentChapter?.subtitle}</span><h1>{currentChapter?.title}</h1></div><div className="story-atlas__progress"><ProgressRing value={currentChapter?.completedRequiredNodes ?? 0} max={currentChapter?.totalRequiredNodes ?? 1} label="Chapter progress"/><span>Chapter progress<small>Boss {currentChapter?.bossStatus}</small></span></div></header>
    <div ref={mapViewport} className="story-atlas__viewport" aria-label="Campaign map. Scroll to explore the territory."><div className="story-atlas__terrain" style={{backgroundImage:`url("${getAssetUrl(currentChapter?.mapAssetId || '')}")`}}><div className="story-atlas__wash"/>
      <svg className="story-atlas__routes" aria-hidden="true">{nodes.flatMap(node=>node.prerequisites.map(id=>{const parent=nodes.find(n=>n.nodeId===id);return parent ? <line key={`${id}-${node.nodeId}`} x1={`${parent.mapPosition.x}%`} y1={`${parent.mapPosition.y}%`} x2={`${node.mapPosition.x}%`} y2={`${node.mapPosition.y}%`} stroke={node.status==='locked' ? '#eee5cf35' : '#e4c788a8'} strokeWidth={node.status==='locked' ? 1 : 2} strokeDasharray={node.status==='locked' ? '3 7' : undefined}/> : null;}))}</svg>
    ...
    <footer className="story-atlas__footer"><div><span className="studio-eyebrow">{recommended ? 'Your next chapter moment' : 'Your territory'}</span><strong>{recommended?.title ?? 'Explore the block. Perfect your story.'}</strong><span className="story-atlas__hint">Select a marker to enter · Scroll to explore</span></div>{recommended && <button className="studio-action studio-action--gold" onClick={()=>setSelectedNodeId(recommended.nodeId)}>Continue story<ArrowRight size={15}/></button>}
