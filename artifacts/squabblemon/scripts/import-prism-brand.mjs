...
for (const [original,name,maxWidth] of entries) {
 ...
 try { bytes=await readFile(input); } catch(error) {
  const duplicateName = {[fist+' (2).png']:fist+' (1).png',[word+' (2).svg']:word+'.svg'}[original];
