...
const bagLogo=new T.TextureLoader().load('../../brand/prismatic/marks/impact-gold.webp');
...
if(face){g.fillStyle=face==='A'?'#192a26':'#9d3233';g.font='bold 62px serif';g.fillText(face,46,104);g.fillText(face==='A'?'♠':'♥',42,162);g.save();g.translate(384,544);g.rotate(Math.PI);g.fillText(face,46,104);g.fillText(face==='A'?'♠':'♥',42,162);g.restore();g.textAlign='center';g.font='150px serif';g.fillText(face==='A'?'♠':'♥',192,330);}else{g.strokeStyle='#987c45';g.lineWidth=1;for(let y=56;y<500;y+=20){g.beginPath();g.moveTo(40,y);g.lineTo(340,y-30);g.stroke();}g.fillStyle='#20332e';g.fillRect(51,190,282,168);g.fillStyle='#e7cc8c';g.textAlign='center';g.font='bold 34px sans-serif';g.font='bold 29px sans-serif';g.fillText('SQUABBLEMON',192,254);g.font='bold 18px sans-serif';g.fillText('HOME COURT EDITION',192,299);}const t=new T.CanvasTexture(c);t.colorSpace=T.SRGBColorSpace;t.anisotropy=8;if(!face){const crest=new Image();crest.onload=()=>{if(contextLost)return;g.fillStyle='#182a26';g.fillRect(39,39,306,466);g.drawImage(crest,56,109,272,280);g.fillStyle='#e7cc8c';g.textAlign='center';g.font='bold 16px sans-serif';g.fillText('HOME COURT EDITION',192,471);t.needsUpdate=true;};crest.src=new URL('../../brand/prismatic/marks/impact-gold.webp',import.meta.url).href;}if(face)crewCardCanvases.push({canvas:c,texture:t});return t;}
...
const initialPose=roomPose();const target=new T.Vector3(...initialPose.slice(3));let yaw=initialPose[0]+.12,pitch=initialPose[1],radius=initialPose[2],desired={yaw:initialPose[0],pitch,radius:initialPose[2],target:target.clone()};
...
if(roomDetails.update(t/1000,dt,reduced))renderer.shadowMap.needsUpdate=true;if(bagImpulse>0&&(t-bagStarted)<5000)renderer.shadowMap.needsUpdate=true;renderer.info.reset();art.render();publishAnchors(t);}
