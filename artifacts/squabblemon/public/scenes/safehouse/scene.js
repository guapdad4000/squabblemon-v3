...
renderer.setPixelRatio(Math.min(devicePixelRatio,innerWidth<600?1.4:1.7));renderer.setSize(innerWidth,innerHeight);renderer.shadowMap.enabled=true;renderer.shadowMap.type=T.PCFSoftShadowMap;renderer.toneMapping=T.ACESFilmicToneMapping;renderer.toneMappingExposure=1.12;host.append(renderer.domElement);
const scene=new T.Scene();scene.background=new T.Color('#090909');scene.fog=new T.Fog('#242321',21,40);const camera=new T.PerspectiveCamera(innerWidth<600?53:39,innerWidth/innerHeight,.1,70);
...
const gold=new T.MeshPhysicalMaterial({color:'#ffc04c',metalness:.85,roughness:.27,clearcoat:.4,clearcoatRoughness:.22});const brass=new T.MeshStandardMaterial({color:'#af772d',metalness:.8,roughness:.4});const black=new T.MeshStandardMaterial({color:'#181927',roughness:.7});const leather=new T.MeshPhysicalMaterial({color:'#962a48',roughness:.65,clearcoat:.2,clearcoatRoughness:.65});const wood=new T.MeshStandardMaterial({map:texture('wood'),roughness:.85});const plaster=new T.MeshStandardMaterial({map:texture('wall'),roughness:1});const ivory=new T.MeshStandardMaterial({color:'#d7cfb4',roughness:.65});
...
const screenMat=new T.MeshStandardMaterial({map:screenTexture,emissiveMap:screenTexture,emissive:'#ffffff',emissiveIntensity:.8,roughness:.28});
...
// Crown pendant.
cyl(.008,.008,1,gold,.2,4.3,-.5);cyl(.49,.59,.43,gold,.2,3.6,-.5);for(let i=0;i<8;i++){let a=i*Math.PI/4;const spike=mesh(new T.ConeGeometry(.13,.36,4),gold,.2+Math.cos(a)*.46,3.98,-.5+Math.sin(a)*.46);spike.rotation.y=-a;}
const glow=new T.MeshBasicMaterial({color:'#ffdb85'});cyl(.48,.48,.02,glow,.2,3.37,-.5);
...
const hemi=new T.HemisphereLight('#ffe6b2','#464377',1.4);scene.add(hemi);const sun=new T.DirectionalLight('#ffd28d',4.5);sun.position.set(8,5,1);sun.target.position.set(-2,0,-2);sun.castShadow=true;sun.shadow.mapSize.set(2048,2048);Object.assign(sun.shadow.camera,{left:-8,right:8,top:8,bottom:-8,near:.1,far:25});sun.shadow.normalBias=.035;scene.add(sun,sun.target);const lamp=new T.PointLight('#ffc164',25,10,2);lamp.position.set(.2,3.22,-.5);scene.add(lamp);const fill=new T.PointLight('#859fdc',18,10,2);fill.position.set(-3,2.4,4);scene.add(fill);const tvLight=new T.PointLight('#8bbfac',3,4,2);tvLight.position.set(-2,1.5,-.9);scene.add(tvLight);const bagLight=new T.SpotLight('#ffc77d',24,7,.52,.7,1.5);bagLight.position.set(2.6,4,-2);bagLight.target=bag;scene.add(bagLight);
...
const target=new T.Vector3(0,1.4,-.65);let yaw=.08,pitch=.29,radius=innerWidth<600?12:10.4,desired={yaw,pitch,radius,target:target.clone()};
...
const raycaster=new T.Raycaster(),pointer=new T.Vector2();const interactive=[{object:tv,key:'story'},{object:bag,key:'training'},{object:cards,key:'cards'},{object:phone,key:'phone'}];
...
function release(e){if(e.type==='pointerup'&&!moved&&!multi){const key=pick(e);if(key){view(key);if(key==='training')punch();}}pointers.delete(e.pointerId);pinch=0;if(!pointers.size)multi=false;const p=[...pointers.values()][0];if(p){px=p.x;py=p.y;}}
...
function view(name){if(!['room','table','lounge','story','training','cards','phone'].includes(name))return;emit({type:'view',view:name});selected=name;document.body.dataset.view=name;document.querySelectorAll('[data-view]').forEach(b=>{const on=b.dataset.view===name;b.classList.toggle('active',on);b.setAttribute('aria-pressed',on);});document.querySelector('#story-panel').hidden=name!=='story';document.querySelector('#punch').hidden=name!=='training';
const v={room:[.08,.29,innerWidth<600?12:10.4,0,1.4,-.65],table:[.05,.66,4.8,.1,.75,.7],lounge:[-.48,.25,6.2,2.1,1,0],story:[.52,.13,innerWidth<600?4.6:4.1,-3.05,1.45,-1.55],training:[.12,.14,innerWidth<600?5.4:4.8,1.35,2,-2.65],cards:[.03,1.05,innerWidth<600?2.5:1.85,.45,.96,1.4],phone:[-.15,1.15,innerWidth<600?2.4:1.75,2.5,.78,3.05]}[name];if(!v)return;desired={yaw:v[0],pitch:v[1],radius:v[2],target:new T.Vector3(...v.slice(3))};}
...
document.querySelectorAll('[data-view]').forEach(b=>b.onclick=()=>view(b.dataset.view));document.querySelector('#reset').onclick=()=>view('room');document.querySelector('#punch').onclick=punch;document.querySelector('#close-story').onclick=()=>view('room');
let night=false;document.querySelector('#light').onclick=e=>{night=!night;e.currentTarget.textContent=night?'☾ Late night':'☀ Golden hour';e.currentTarget.setAttribute('aria-pressed',night);sun.intensity=night?.2:4.5;sun.color.set(night?'#a49fe6':'#ffd28d');hemi.intensity=night?.65:1.4;sky.color.set(night?'#233c53':'#ffcd84');lamp.intensity=night?38:25;screenMat.emissiveIntensity=night?1.15:.8;tvLight.intensity=night?6:3;};
...
addEventListener('resize',()=>{camera.aspect=innerWidth/innerHeight;camera.fov=innerWidth<600?53:39;camera.updateProjectionMatrix();renderer.setSize(innerWidth,innerHeight);art.resize();if(selected==='room')desired.radius=innerWidth<600?12:10.4;});
...
function publishAnchors(t){if(parent===window||t-lastAnchorUpdate<100)return;lastAnchorUpdate=t;const anchors=interactive.map(item=>{anchorPoint.set(0,item.key==='story'?1.75:item.key==='training'?.8:.08,0);item.object.localToWorld(anchorPoint);anchorPoint.project(camera);return {id:item.key,x:(anchorPoint.x+1)*50,y:(1-anchorPoint.y)*50,visible:anchorPoint.z>-1&&anchorPoint.z<1&&Math.abs(anchorPoint.x)<.87&&Math.abs(anchorPoint.y)<.65};});emit({type:'anchors',anchors});}
function animate(t){requestAnimationFrame(animate);if(contextLost||document.hidden)return;const dt=Math.min((t-lastTime)/1000,.05);lastTime=t;const speed=reduced?1:1-Math.exp(-dt*5);yaw+=(desired.yaw-yaw)*speed;pitch+=(desired.pitch-pitch)*speed;radius+=(desired.radius-radius)*speed;target.lerp(desired.target,speed);camera.position.set(target.x+Math.sin(yaw)*Math.cos(pitch)*radius,target.y+Math.sin(pitch)*radius,target.z+Math.cos(yaw)*Math.cos(pitch)*radius);camera.lookAt(target);
if(!reduced){dust.position.y=Math.sin(t*.00015)*.09;const age=(t-bagStarted)/1000;bagPivot.rotation.z=Math.sin(age*5.5)*bagImpulse*Math.exp(-age*1.35)+Math.sin(t*.0007)*.004;bagPivot.rotation.x=Math.sin(age*4)*bagImpulse*.3*Math.exp(-age*1.35);}art.render();publishAnchors(t);}
