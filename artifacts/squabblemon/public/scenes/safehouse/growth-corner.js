...
export function createGrowthCorner({ wood, brass }) {
  ...
  pot(-.12,.7,-.04,.19,.29,cream);pot(.29,.7,.16,.115,.19,clay);pot(-.57,0,.17,.18,.3,clay);
  ...
  const growLight=new T.PointLight('#ddffc1',2.2,2.8,2);growLight.position.set(-.35,1.95,.7);root.add(growLight);
  const dummy=new T.Object3D();const leaves=new T.InstancedMesh(leaf,green,21);leaves.castShadow=leaves.receiveShadow=true;root.add(leaves);
  for(let i=0;i<9;i++){
