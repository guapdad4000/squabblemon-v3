...
async function run(width:number,height:number){
 ...
 const deck={id:'my-crew',name:'My Mixed Gang',cardIds:[...ROOKIE_CORE_IDS],heroCardId:'hooper',recipeId:null,valid:true,issues:[]};
 ...
 try{
  ...
  for(const kind of (process.env.DRAFT_ONLY ? ['draft'] : ['freeze','fair','neighborhood','boss','draft']) as ActivityId[]){
   ...
   await finish(page);await page.getByLabel('Dr. Fade advice').waitFor();assert.equal(await page.getByRole('button',{name:'Retry Save',exact:true}).count(),0);if(kind==='draft')await page.screenshot({path:`../../screenshots/gameplay-result-${width}.png`,fullPage:true});
  }
