...
function Studio() {
 ...
 return <main className="foil-studio"><header><span>SQUABBLEMON / COLLECTOR SERIES</span><h1>THE FINISH ROOM</h1><p>Seven stocks. Two custom editions. Move across the card to catch the light.</p></header>
 {inspect && <CardInspector card={card} bootstrap={{profile:{ownedCardIds:[card.catalogId],equippedVariants:{},cardProgression:{[card.catalogId]:{xp:2800,level:8,moveTier:2}},styleShards:200,ownedVariants:[],savedDecks:[]}}} onClose={()=>setInspect(false)}/>}<section className="foil-stage"><div className="foil-stage__portrait">{mounted && <CardView card={card} variantId={variant === 'base' ? undefined : card.id+':'+variant} isInspector presentationOnly disableLayout className="w-full aspect-[63/88]"/>}</div><aside><span>COLLECTOR FINISH / {String(tier+1).padStart(2,'0')}</span><h2>{cardFinishLabel(card.rarity, variant === 'base' ? null : variant as 'tagged' | 'chrome')}</h2><p>{CARD_RARITY_DEFINITIONS[card.rarity].label} · {card.name}</p><nav aria-label="Edition">{['base','tagged','chrome'].map(v=><button key={v} aria-pressed={variant===v} onClick={()=>setVariant(v)}>{v}</button>)}</nav><button className="mount-toggle" onClick={()=>setMounted(v=>!v)}>{mounted?'Close display':'Open display'}</button><button style={{marginLeft:8}} onClick={()=>setInspect(true)}>Card details</button></aside></section>
