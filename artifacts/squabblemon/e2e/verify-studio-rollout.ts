...
async function run(width:number,height:number){
 ...
 async function shot(name:string){await page.evaluate(async()=>{await Promise.all(Array.from(document.images).filter(i=>!i.complete).map(i=>new Promise(resolve=>{i.onload=resolve;i.onerror=resolve;setTimeout(resolve,2000)})))});const brokenImages=await page.locator('img:visible').evaluateAll(images=>images.filter(image=>{const img=image as HTMLImageElement,rect=img.getBoundingClientRect();return rect.bottom>0&&rect.top<innerHeight&&rect.right>0&&rect.left<innerWidth&&(!img.complete||img.naturalWidth===0);}).map(image=>(image as HTMLImageElement).currentSrc||(image as HTMLImageElement).src));assert.deepEqual(brokenImages,[],`${name} broken images`);assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false,`${name} overflow`);await page.screenshot({path:`../../screenshots/studio-${name}-${width}.png`,fullPage:true});}
  try{
  ...
  await page.route('**/api/player/**',async route=>{const req=route.request(),path=new URL(req.url()).pathname;
   if(path.endsWith('/bootstrap'))return route.fulfill({json:bootstrap()});if(path.endsWith('/story'))return route.fulfill({json:campaign});
    if(path.endsWith('/matches')&&req.method()==='POST')return route.fulfill({status:503,json:{error:'Story start captured by fixture'}});
   ...
   if(process.env.STUDIO_STORY_FALLBACK_ONLY){
    owned.splice(0);
    await goto('/story/play/receipts-on-camera','.activity-stage');
    assert.equal(await page.getByRole('button',{name:'Build your gang',exact:true}).count(),0,'Story never strands a player in deck building');
    const startRequest=page.waitForRequest(request=>new URL(request.url()).pathname.endsWith('/api/player/matches')&&request.method()==='POST');
    ...
    assert.equal(body.storyNodeId,'receipts-on-camera');
    assert.equal(body.playerDeckId,'block');
  ...
  await goto('','.safehouse-stage');await page.locator('.safehouse-stage[data-scene-ready="true"]').waitFor({timeout:45000});await page.waitForTimeout(1000);await shot('home');assert.equal(await page.locator('.safehouse-room-stations').count(),0);assert.ok(await page.locator('.safehouse-room-markers button:visible').count()>=5);await page.getByRole('button',{name:'Explore your crew cards',exact:true}).click();await page.getByRole('link',{name:'Build your crew',exact:true}).waitFor();await page.waitForTimeout(600);await shot('home-station');await page.getByRole('button',{name:'Back to the room',exact:true}).click();await page.getByRole('button',{name:'Reset room camera',exact:true}).click();await page.getByRole('button',{name:'Switch to golden hour',exact:true}).click();await page.getByRole('button',{name:'Switch to late night',exact:true}).waitFor();
  assert.deepEqual(errors,[]);assert.deepEqual(failedAssets,[]);console.log(`${width}px: destinations, assets, claim retry, purchase recovery, story, and Safehouse rendering passed.`);
 }catch(e){await page.screenshot({path:`../../screenshots/studio-debug-${width}.png`,fullPage:true});console.log({errors,failedAssets,url:page.url(),body:(await page.locator('body').innerText()).slice(-6000)});throw e;}finally{await browser.close();}
}
