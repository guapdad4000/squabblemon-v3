...
try {
 for (const width of [1440, 768, 390, 320]) {
  ...
  await page.goto(base+'/e2e/notifications.fixture.html');
 ...
 await page.getByRole('button',{name:/Notifications,/}).click();
 await expect(page.locator('.notification-item').filter({hasText:'Your free 50 Clout is ready'})).toHaveCount(0);
 await page.keyboard.press('Escape');
 claimed = false; date = '2026-09-26';
 await page.reload();
 await page.getByRole('button',{name:/Notifications,/}).click();
 await expect(page.locator('.notification-item').filter({hasText:'Straight to the Back'})).toBeVisible();
 await expect(page.locator('.notification-item').filter({hasText:'Your free 50 Clout is ready'})).toBeVisible();
 await page.goto(base+'/e2e/notifications.fixture.html?player=second-player');
 await expect(page.getByRole('button',{name:'Inspect Kyle'}).locator('.attention-mark')).toHaveCount(1);
 console.log('PASS: responsive header, persistent item reads, dismissible alerts without claiming rewards/mail, accessible dialog dismissal, daily claim state.');
