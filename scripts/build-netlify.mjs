...
if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
  ...
  run(['scripts/check-online.mjs','artifacts/squabblemon/src/multiplayer.test.ts','artifacts/squabblemon/src/kyle.test.ts','artifacts/squabblemon/src/stockz.test.ts','artifacts/api-server/src/lib/collectionEconomy.test.ts'],env);
  run(['--import',pathToFileURL(createRequire(import.meta.url).resolve('tsx', { paths: [path.resolve('artifacts/api-server')] })).href,'--test','artifacts/api-server/src/lib/cosmetics.test.ts'],{ ...env, DATABASE_URL: '' });
