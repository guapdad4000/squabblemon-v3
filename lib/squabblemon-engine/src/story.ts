...
export type StoryReward = { readonly kind: "currency" | "card" | "chapter-key" | "pack-ticket" | "cosmetic" | "character-unlock"; readonly id: string; readonly amount: number };
...
const encounter = (id: string, name: string, portraitAssetId: string, cards: readonly string[], behaviorProfile: string, cinematicMetadata: StoryCinematic, extra: Pick<StoryEncounterSnapshot, "modifiers" | "phases" | "passive" | "starObjectives"> = {}): StoryEncounterSnapshot => ({
...
// Major story nodes (chapter finale reward beats) grant a ten-pack bundle.
// Sized to match the upgraded ten-pull: 10 tickets → 60 rewards from the
// punching bag. The chapter-finale reward array in this file and the runtime
// additions in seasonChapters.ts both use this constant so the campaign
// stays in lockstep with the gacha economy.
...
export const storyContent = validateStoryContent({ version: 4, chapters: [{ ...blockPartyChapter, nodes: blockPartyChapter.nodes.map((node): StoryNode => {
