  ...
  for (const chapter of content.chapters) {
    ...
    for (const node of chapter.nodes) {
      ...
      if (node.kind === "battle") {
        ...
        if (
          Object.values(modifiers.startingHype ?? {}).some((value) => !Number.isInteger(value) || value < 0) ||
          ...
          (modifiers.roundHypeDeltas ?? []).some((item) => !Number.isInteger(item.round) || item.round < 1 || item.round > 6 || !Number.isInteger(item.amount)) ||
