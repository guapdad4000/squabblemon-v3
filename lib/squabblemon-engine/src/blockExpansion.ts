...
export const BLOCK_EXPANSION = [
  ...
  ['mural', 'mural-apprentice', 'Mural Apprentice', 'Uncommon', 'Poison', 2, 2, 'Fresh Color', 'On Reveal: If another friendly card with a different type is here, apply Lock to the highest-Hands enemy here. Otherwise gain +1 Hands.', 'Disruption'],
  ['chessregular', 'chess-regular', 'Chess Regular', 'Common', 'Dark', 3, 3, 'Quiet Fork', 'On Reveal: If exactly one enemy is here, apply 1 Burn. If they have 3 or more Hands, apply 2 Burn instead.', 'Disruption'],
  ['gardener', 'rooftop-gardener', 'Rooftop Gardener', 'Common', 'Plant', 1, 1, 'Plant Bond', 'Ongoing: While Rooftop Gardener is in your hand, your other Plant characters gain +1 Hand at round end.', 'Tempo', 'Plant'],
  ['piratedj', 'pirate-radio-dj', 'Pirate Radio DJ', 'Epic', 'Electric', 3, 2, 'Citywide Signal', 'On Reveal: Give your lowest-Hands ally in each other district +1 Hand. Ongoing: In hand, your other Electric characters gain +1 Hand at round end. On board, your second Electric character played each round gains +1 Hand and refunds 1 Motion.', 'Support', 'Electric'],
  ...
  ['nightmedic', 'night-shift-medic', 'Night Shift Medic', 'Epic', 'Light', 3, 4, 'All Clear', 'On Reveal: Cleanse Freeze, Silence, Burn, Weaken, and Lock from every other ally here. Ongoing: While on board, the first Light character you cleanse each round gains +2 Hands.', 'Support'],
  ...
  ['conductor', 'last-train-conductor', 'Last Train Conductor', 'Mythical', 'Water', 4, 3, 'Last Stop', 'Move your lowest-Hands other ally here to your weakest other district. If moved, Cleanse it and give it +3 Hands, or +4 if it is Water.', 'Movement'],
  ...
  ['bigzoey', 'big-zoey', 'Big Zoey', 'Legendary', 'Earth', 3, 3, 'Hold the Block', 'Give your lowest-Hands other ally here +2 Hands and the lowest-Hands enemy here -2 Hands.', 'Disruption'],
  ...
  ['partytitan', 'block-party-titan', 'Block Party Titan', 'Mythical', 'Earth', 6, 7, 'Everybody Outside', 'Give your weakest other friendly card in each district +1 Hand.', 'Support'],
] as const;
...
export const expansionCards: Record<string, Card> = Object.fromEntries(BLOCK_EXPANSION.map(
  ([engineId, id, name, , type, cost, power, ability, effect, role, elementalBond]) => [engineId, {
    ...
    abilityUpgrades: [2, 5, 8].map((unlockLevel, index) => ({
      ...
      description: elementalBond && effect.startsWith('Ongoing:')
