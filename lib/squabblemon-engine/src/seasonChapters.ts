...
function buildChapter(plan: ChapterPlan): StoryChapter {
  ...
  const nodes: StoryNode[] = plan.beats.map((beat) => {
    ...
    const base = {
      ...
      teaching: { tips: [beat.opponent ? 'Win the fade to advance this story.' : 'Continue the story at your own pace.'], focusMechanics: [beat.battleType === 'rule-twist' ? 'district rules' : 'story progression'], focusCards: ['cornball'] },
    };
