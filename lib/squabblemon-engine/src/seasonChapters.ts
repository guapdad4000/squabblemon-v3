...
function buildChapter(plan: ChapterPlan): StoryChapter {
  ...
  const nodes: StoryNode[] = plan.beats.map((beat) => {
    ...
    const modifiers = reduceRivalMotion ? { ...beat.modifiers, roundMotionDeltas: [
      ...(beat.modifiers?.roundMotionDeltas ?? []),
      ...Array.from({ length: roundLimit - 1 }, (_, index) => ({ round: index + 2, owner: 'cpu' as const, amount: -1 })),
    ] } : beat.modifiers ?? {};
  ...
  { id: 'side-show', order: 4, title: 'Chapter Four: Side Show', subtitle: 'A clip is shorter than the truth.',
    description: 'Trace an edited recording through Snitch, Scammer and the block.', map: 'civic-summit', venue: 'civic-hill-climb', beats: [
      ...
      { id: 'snitchs-price', title: "Snitch's Price", opponent: 'Snitch', deck: deckReceipts,
        before: [['Snitch', "My price is a public interview. No edits from either of us."], ['Ganger Red', "A fade gets you the microphone. It does not make your footage true."], ['Snitch', "Then win the microphone."]],
      ...
      { id: 'snitchs-verdict', title: "Snitch's Verdict", opponent: 'Snitch', deck: deckMedia, battleType: 'boss', phases: [phase('show', 1, 1), phase('read', 3, 1), phase('verdict', 5, 1)],
        before: [['Snitch', "Red asked for the whole cut. That includes what I did with the address."], ['Ganger Red', "The fade decides who speaks first. The file decides what they say."], ['Snitch', "Fine. Camera stays on."]],
  ...
  { id: 'old-heads-know', order: 5, title: 'Chapter Five: Old Heads Know', subtitle: 'Every secret had a cost.',
    description: 'Bring the family to one table and hear the full rescue story.', map: 'old-town', venue: 'corner-store-court', beats: [
      ...
      { id: 'baby-mommas-truth', title: "Baby Momma's Truth", opponent: 'Baby Momma', deck: deckFamily,
        ...
        after: [['Baby Momma', "You won a fade. You did not win an answer about us."], ['Cracked Head', "I heard you."]] },
      ...
      { id: 'alleys-confession', title: "Alley's Confession", opponent: 'Alley Runner', deck: deckRed, optional: true, afterNode: 'wifeys-stand',
        before: [['Alley Runner', "Everybody keeps asking me to carry messages they will not say themselves."], ['Cornball', "Could you carry my appeal to the vending company?"], ['Alley Runner', "Win a side fade, then ask me again."]],
      ...
      { id: 'og-uncles-verdict', title: "OG Uncle's Account", opponent: 'OG Uncle', deck: deckFamily, battleType: 'boss',
        modifiers: { handSize: { cpu: 4 }, roundMotionDeltas: [2, 3, 4, 5, 6].map(round => ({ round, owner: 'cpu', amount: -1 })) },
  ...
  { id: 'the-function', order: 6, title: 'Chapter Six: The Function', subtitle: 'The room is bigger than the rivalry.',
    description: 'Keep the fundraiser open while the brothers agree to fair entry.', map: 'festival-street', venue: 'crown-rooftop-court', beats: [
      { id: 'function-opening', title: 'The Function', opponent: 'Bottle Girl', deck: deckRed,
        before: [['Bottle Girl', "Welcome to the fundraiser. Setup gang gets fed before the headliners."], ['Cornball', "I brought a clipboard and an appetite. Which one checks in first?"], ['Bottle Girl', "Win the opening table and I will decide."]],
      ...
      { id: 'the-bar-fight', title: 'The Bar Fight', opponent: 'Bottle Girl', deck: deckFamily, battleType: 'rule-twist', modifiers: { startingMotion: { player: 3 }, roundMotionDeltas: [2, 3, 4, 5].map(round => ({ round, owner: 'cpu', amount: -2 })), laneLocks: [{ round: 3, owner: 'both', lanes: [1] }] },
  ...
  { id: 'return-of-the-block', order: 7, title: 'Chapter Seven: Return of the Block', subtitle: 'The list belongs to everyone.',
    description: 'Restore the open bracket and make Blue answer for changing it.', map: 'crown-court', venue: 'crown-rooftop-court', beats: [
      ...
      { id: 'baby-mommas-boundary', title: "Baby Momma's Boundary", opponent: 'Baby Momma', deck: deckFamily, optional: true, afterNode: 'blues-guilt',
        before: [['Baby Momma', "The whole block has opinions about our child. The child did not ask for a microphone."], ['Cornball', "I can make a quiet sign."], ['Baby Momma', "Make it after a side fade. Then put the pen down."]],
  ...
  { id: 'the-crown', order: 8, title: 'Chapter Eight: The Crown', subtitle: 'The Open belongs to the people who play it.',
    description: 'Cross the neighborhood, win the final and defend the Open as the rooftop changes owners.', map: 'sunset-block', venue: 'crown-rooftop-court', beats: [
      ...
      { id: 'crown-cornballs-table', title: "Cornball's Table", opponent: 'Cornball', deck: deckReceipts, venue: 'corner-store-court',
        before: [['Cornball', "The vending machine finally refunded my three dollars. In store credit. For that machine."], ['Alley Runner', "You can buy water after the fade."], ['Cornball', "I am buying justice first. Shuffle."]],
        after: [['Cornball', "You win. I am spending the credit on water for the setup gang."], ['Alley Runner', "That is the first sensible thing the machine has done."]] },
      ...
      { id: 'crown-baby-mommas-terms', title: "Baby Momma's Terms", opponent: 'Baby Momma', deck: deckFamily,
        ...
        after: [['Baby Momma', "The fade is settled. Parenting is a separate conversation."], ['Cracked Head', "I am staying for that conversation."]] },
