...
const applyOngoingRoundEndHandEffects = (m: Match): Match => {
  ...
  for (const hand of [result.playerHand, result.cpuHand] as const) {
    for (const card of hand) {
      ...
      if (!bond) continue;
      const allies = result.boards.flat().filter(c => !c.hazard && c.owner === card.owner && c.type === bond && c.instanceId !== card.instanceId);
...
function resolveAbility(match: Match, source: CardInstance): Match {
  ...
  if (source.cardId === 'rastamon') { const t = lowest(inLane(m, source.owner, l).filter((c) => c.instanceId !== source.instanceId && (c.statuses.frozen || c.statuses.silenced))); if (t) { targetIds.add(t.instanceId); m = modify(m, t.instanceId, (c) => ({ ...c, statuses: cleanseStatuses(c.statuses), powerModifier: c.powerModifier + 2, lastEffectNote: 'Natural Cure: cleansed, +2 Hands.' })); note('Natural Cure cleansed an ally and gave it +2.'); } else note('Natural Cure found no status to cleanse.'); }
    ...
    const succeeds = source.cardId === 'youngbull' ? enemiesHere.length > 0
    ...
    if (target) {
      targetIds.add(target.instanceId);
      m = modify(m, target.instanceId, c => ({ ...c, statuses: cleanseStatuses(c.statuses), powerModifier: c.powerModifier + amount, lastEffectNote: `${source.ability}: cleansed, +${amount} Hands.` }));
    }
    ...
    m = summonCard(m, source.owner, destination, SUMMON_TEMPLATES.steward, 'steward', source, false, 'steward-blue');
    ...
    if (['homelessyn', 'sportsprodigy', 'fein', 'alchy', 'divorceddad', 'failedathlete'].includes(id)) {
      ...
      for (const target of m.boards.flat().filter(c => !c.hazard && c.owner === source.owner && c.instanceId !== source.instanceId && c.kind !== 'support')) cleanse(target, 0);
...
export function nextRound(match: Match): Match {
  ...
  if (match.round >= 6) {
    ...
    return addEvent(complete, complete, { type: 'match-complete', owner: 'player', note: 'The fade is complete.' });
  }
...
function replayPlayerMoves(initial: Match, moves: readonly TranscriptMove[]): Match {
  if (moves.length < 6 || moves.length > MAX_MATCH_MOVES) throw new Error('A fade needs six round endings and at most 64 actions');
  const multiCardTurns = moves.some(move => move.endTurn !== undefined);
  validateTurnRules(moves, multiCardTurns ? 2 : 1);
  if (!multiCardTurns && moves.length !== 6) throw new Error('A legacy fade transcript needs six moves');
  let match = initial;
