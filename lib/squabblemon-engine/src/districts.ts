...
export function createDistrictSnapshot(seed: string): DistrictSnapshot {
  ...
  // Story v1 seeds keep their authored location pool when new districts launch.
  // This preserves campaign difficulty and replays; other modes use the full pool.
  const pool = [...(seed.startsWith('story-node-v1:') ? DISTRICT_CATALOG.slice(0, 16) : DISTRICT_CATALOG)];
